...
public class printwip extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request,
        ...
        try {
            ...
            ws.logMessage("DEBUG",className,"Proof print.          UNIQUE_ID <" + uniqueId + ">");
            ws.logMessage("DEBUG",className,String.format("IDS Request[<IdsConfig=%s>, <UniqueId=%s>, <IdsRequestUID=%s>, <IdsRequestPwd=%s>, <IdsProofPrintType=%s>, <IdsRequestProof=%s>]",
                                                          ws.getIdsConfig(),          uniqueId,       ws.getIdsRequestUID(), ws.getIdsRequestPWD(),ws.getIdsProofPrintType(), ws.getIdsRequestProof()));
            
            res = cli.printWIPEntry(ws.getIdsConfig(), uniqueId, ws.getIdsRequestUID(), ws.getIdsRequestPWD(),ws.getIdsProofPrintType(),ws.getIdsRequestProof());
        
            results = res.getResults();

            if (results.getResult() != 0) {
                ws.logMessage("ERROR",className,
                           "Unable to PROOF PRINT UNIQUE_ID <" + uniqueId +
                           ">");
                response.setContentType("text/html");
                PrintWriter out = response.getWriter();
